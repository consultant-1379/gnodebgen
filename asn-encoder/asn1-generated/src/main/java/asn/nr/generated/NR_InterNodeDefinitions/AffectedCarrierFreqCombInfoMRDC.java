...
public class AffectedCarrierFreqCombInfoMRDC extends Asn1Seq {
      ...
      throws Asn1Exception, java.io.IOException
   {
      ...
      buffer.encodeBit ((affectedCarrierFreqCombMRDC != null), null);

      // encode victimSystemType

      if (victimSystemType != null) {
         buffer.getContext().eventDispatcher.startElement("victimSystemType", -1);

         victimSystemType.encode (buffer);

         buffer.getContext().eventDispatcher.endElement("victimSystemType", -1);
      }
      else throw new Asn1MissingRequiredException ("victimSystemType");

      // encode interferenceDirectionMRDC

      if (interferenceDirectionMRDC != null) {
         buffer.getContext().eventDispatcher.startElement("interferenceDirectionMRDC", -1);

         interferenceDirectionMRDC.encode (buffer);

         buffer.getContext().eventDispatcher.endElement("interferenceDirectionMRDC", -1);
      }
      else throw new Asn1MissingRequiredException ("interferenceDirectionMRDC");

      // encode affectedCarrierFreqCombMRDC

      if (affectedCarrierFreqCombMRDC != null) {
         buffer.getContext().eventDispatcher.startElement("affectedCarrierFreqCombMRDC", -1);

         affectedCarrierFreqCombMRDC.encode (buffer);
