...
public class SRS_CarrierSwitching extends Asn1Seq {
      ...
      throws Asn1Exception, java.io.IOException
   {
      ...
      buffer.encodeBit ((srs_SwitchFromServCellIndex != null), null);
      buffer.encodeBit ((srs_TPC_PDCCH_Group != null), null);
      buffer.encodeBit ((monitoringCells != null), null);

      // encode srs_SwitchFromServCellIndex

      if (srs_SwitchFromServCellIndex != null) {
         buffer.getContext().eventDispatcher.startElement("srs_SwitchFromServCellIndex", -1);

         srs_SwitchFromServCellIndex.encode (buffer, 0, 31);

         buffer.getContext().eventDispatcher.endElement("srs_SwitchFromServCellIndex", -1);
      }

      // encode srs_SwitchFromCarrier

      if (srs_SwitchFromCarrier != null) {
         buffer.getContext().eventDispatcher.startElement("srs_SwitchFromCarrier", -1);

         srs_SwitchFromCarrier.encode (buffer);

         buffer.getContext().eventDispatcher.endElement("srs_SwitchFromCarrier", -1);
      }
      else throw new Asn1MissingRequiredException ("srs_SwitchFromCarrier");

      // encode srs_TPC_PDCCH_Group

      if (srs_TPC_PDCCH_Group != null) {
         buffer.getContext().eventDispatcher.startElement("srs_TPC_PDCCH_Group", -1);

         srs_TPC_PDCCH_Group.encode (buffer);

         buffer.getContext().eventDispatcher.endElement("srs_TPC_PDCCH_Group", -1);
      }

      // encode monitoringCells

      if (monitoringCells != null) {
         buffer.getContext().eventDispatcher.startElement("monitoringCells", -1);

         monitoringCells.encode (buffer);
