/**
 * This file was generated by the Objective Systems ASN1C Compiler
 * (http://www.obj-sys.com).  Version: 7.2.4, Date: 05-Aug-2021.
 */
package asn.lte.generated.EUTRA_RRC_Definitions;

import com.objsys.asn1j.runtime.*;

public class RS_ConfigSSB_NR_r15 extends Asn1Seq {
   private static final long serialVersionUID = 55;
   static {
      _setKey (_EUTRA_RRC_DefinitionsRtkey._rtkey);
   }

   public String getAsn1TypeName()  {
      return "RS-ConfigSSB-NR-r15";
   }

   public MTC_SSB_NR_r15 measTimingConfig_r15;
   public RS_ConfigSSB_NR_r15_subcarrierSpacingSSB_r15 subcarrierSpacingSSB_r15 = null;
   public Asn1OpenExt extElem1;

   public RS_ConfigSSB_NR_r15 () {
      super();
      init();
   }

   /**
    * This constructor sets all elements to references to the 
    * given objects
    */
   public RS_ConfigSSB_NR_r15 (
      MTC_SSB_NR_r15 measTimingConfig_r15_,
      RS_ConfigSSB_NR_r15_subcarrierSpacingSSB_r15 subcarrierSpacingSSB_r15_
   ) {
      super();
      measTimingConfig_r15 = measTimingConfig_r15_;
      subcarrierSpacingSSB_r15 = subcarrierSpacingSSB_r15_;
   }

   public void init () {
      measTimingConfig_r15 = null;
      subcarrierSpacingSSB_r15 = null;
      extElem1 = null;
   }

   public int getElementCount() { return 3; }


   public Object getElementValue(int index){
      switch(index)  {
         case 0: return measTimingConfig_r15;
         case 1: return subcarrierSpacingSSB_r15;
         case 2: return extElem1;
         default: return null;
      }
   }


   public String getElementName(int index){
      switch(index)  {
         case 0: return "measTimingConfig-r15";
         case 1: return "subcarrierSpacingSSB-r15";
         case 2: return null;
         default: return null;
      }
   }


   public void decode (Asn1PerDecodeBuffer buffer)
      throws Asn1Exception, java.io.IOException
   {
      init ();

      // extension bit

      boolean extbit = buffer.decodeBit ("extbit");

      // decode measTimingConfig_r15

      buffer.getContext().eventDispatcher.startElement("measTimingConfig_r15", -1);

      measTimingConfig_r15 = new MTC_SSB_NR_r15();
      measTimingConfig_r15.decode (buffer);
      buffer.getContext().eventDispatcher.endElement("measTimingConfig_r15", -1);

      // decode subcarrierSpacingSSB_r15

      buffer.getContext().eventDispatcher.startElement("subcarrierSpacingSSB_r15", -1);

      {
         int tval = RS_ConfigSSB_NR_r15_subcarrierSpacingSSB_r15.decodeEnumValue (buffer);
         subcarrierSpacingSSB_r15 = RS_ConfigSSB_NR_r15_subcarrierSpacingSSB_r15.valueOf (tval);
      }
      buffer.getContext().eventDispatcher.endElement("subcarrierSpacingSSB_r15", -1);

      if (extbit) {

         int bitcnt = buffer.decodeSmallLength(), i = 0;
         long bitidx = buffer.getBitOffset() + bitcnt;
         boolean[] bitmap = new boolean [bitcnt];

         while (i < bitcnt) {
            bitmap[i++] = buffer.decodeBit ("unknown");
         }

         i = 0;

         // decode unknown extension elements

         if (i < bitcnt) {
            Asn1OpenType openType = null;
            extElem1 = new Asn1OpenExt();
            int j = 0;
            while (i < bitcnt) {
               if (bitmap[i]) {
                  buffer.getContext().eventDispatcher.startElement("...", -1);

                  openType = extElem1.decodeOpenType (buffer, true, j++);

                  buffer.invokeCharacters (openType.toString());
                  buffer.getContext().eventDispatcher.endElement("...", -1);
               }
               else {
                  extElem1.setOpenType (null, j++);
               }
               i++;
            }
         }
         buffer.byteAlign ();

      }

   }

   public void print (java.lang.StringBuilder _sb, String _varName, int _level)
   {
      indent (_sb, _level);
      _sb.append(_varName).append(" {\n");
      if (measTimingConfig_r15 != null) measTimingConfig_r15.print (_sb, "measTimingConfig_r15", _level+1);
      if (subcarrierSpacingSSB_r15 != null) subcarrierSpacingSSB_r15.print (_sb, "subcarrierSpacingSSB_r15", _level+1);
      if (extElem1 != null) extElem1.print (_sb, "extElem1", _level+1);
      indent (_sb, _level);
      _sb.append ("}\n");
   }

}
